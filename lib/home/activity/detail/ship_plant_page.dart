...
import 'package:ai_flutter/home/activity/detail/input_ship_plant_page.dart';
...
class ShipPlantPage extends StatelessWidget {
  ShipPlantPage(this._inputActivityForm, this.activityText);
  ...
  Future<Null> refreshList() async {
    ...
    _shipPlantBloc.add(
      LoadShipPlantData(_inputActivityForm),
    );
    print("refresh");
    return null;
  }
  ...
  Future<void> editButton(BuildContext context, var value) async {
    try {
      ...
      print(_shipPlantBloc.historyList[value].id);
      var rs = await ActivityAPI()
          .getTransportData(_shipPlantBloc.historyList[value].id);
      var jsonRes = json.decode(rs.body);
      ShipPlantModel dataForEdit = ShipPlantModel.fromJson(jsonRes[0]);
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => InputShipPlantPage(
            _inputActivityForm,
            activityText,
            dataForEdit,
          ),
  ...
  ShipPlantBloc _shipPlantBloc;
  @override
  Widget build(BuildContext context) {
    _shipPlantBloc = BlocProvider.of<ShipPlantBloc>(context);
    return Scaffold(
      body: RefreshIndicator(
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              BlocBuilder<ShipPlantBloc, ShipPlantState>(
                builder: (context, state) {
                  Widget widgetBox = SizedBox();
                  if (state is ShipPlantInitial) {
                    _shipPlantBloc.add(
                      LoadShipPlantData(_inputActivityForm),
                    );
                  } else if (state is ShipPlantLoaded) {
                    title = _shipPlantBloc.title + " / $activityText";
                    widgetBox = Column(
                      children: [
                        ListViewTimeLineWidget(
                          data: _shipPlantBloc.historyList,
                                ...
                                builder: (context) {
                                  return AlertDialog(
                                    ...
                                    actions: [
                                      FlatButton(
                                        onPressed: () {
                                          _shipPlantBloc.add(DeleteShipPlant(
                                              _shipPlantBloc
                  ...
                  } else if (state is ShipPlantNoData) {
                    widgetBox = Column(
                      children: [
                        Text("ไม่มีข้อมูล"),
                      ],
                    );
                  } else if (state is ShipPlantError) {
                    widgetBox = Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                  ...
                  } else if (state is ShipPlantLoading) {
                    widgetBox = Column(
                      children: [
                        Center(
                          child: new Container(
                  ...
                  return Column(
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          _shipPlantBloc.startDate == null
                              ? SizedBox()
                              : PickTime(
                                  startDate: _shipPlantBloc.startDate,
                                  endDate: _shipPlantBloc.endDate,
                                  focusedStartDay: (value) {
                                    _shipPlantBloc
                                        .add(UpdateShipPlantStartDate(value));
                                  },
                                  focusedLastDay: (value) {
                                    print(value);
                                    _shipPlantBloc
                                        .add(UpdateShipPlantEndDate(value));
                                  },
                                ),
                          _shipPlantBloc.stationSelect == null
                              ? SizedBox()
                              : Container(
                                  height: 40,
                                  width: 100,
                                  ...
                                  child: DropdownButton<PlotDetail>(
                                    value: _shipPlantBloc.stationSelect,
                                    ...
                                    onChanged: (PlotDetail newValue) {
                                      _shipPlantBloc.add(
                                          UpdateShipPlantStation(newValue));
                                    },
                                    items: _shipPlantBloc.stationList
