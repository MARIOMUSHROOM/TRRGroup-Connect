...
import 'package:ai_flutter/home/activity/detail/input_cut_plant_page.dart';
...
class CutPlantPage extends StatelessWidget {
  CutPlantPage(this._inputActivityForm, this.activityText);
  ...
  Future<Null> refreshList() async {
    ...
    _cutPlantBloc.add(
      LoadCutPlantData(_inputActivityForm),
    );
    print("refresh");
    return null;
  }
  ...
  Future<void> editButton(BuildContext context, var value) async {
    try {
      ...
      print(_cutPlantBloc.historyList[value].id);
      var rs = await ActivityAPI()
          .getLeavesData(_cutPlantBloc.historyList[value].id);
      var jsonRes = json.decode(rs.body);
      CutPlantModel dataForEdit = CutPlantModel.fromJson(jsonRes[0]);
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => InputCutPlantPage(
            _inputActivityForm,
            activityText,
            dataForEdit,
          ),
  ...
  CutPlantBloc _cutPlantBloc;
  @override
  Widget build(BuildContext context) {
    _cutPlantBloc = BlocProvider.of<CutPlantBloc>(context);
    return Scaffold(
      body: RefreshIndicator(
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              BlocBuilder<CutPlantBloc, CutPlantState>(
                builder: (context, state) {
                  Widget widgetBox = SizedBox();
                  if (state is CutPlantInitial) {
                    _cutPlantBloc.add(
                      LoadCutPlantData(_inputActivityForm),
                    );
                  } else if (state is CutPlantLoaded) {
                    title = _cutPlantBloc.title + " / $activityText";
                    widgetBox = Column(
                      children: [
                        ListViewTimeLineWidget(
                          data: _cutPlantBloc.historyList,
                                ...
                                builder: (context) {
                                  return AlertDialog(
                                    ...
                                    actions: [
                                      FlatButton(
                                        onPressed: () {
                                          _cutPlantBloc.add(DeleteCutPlant(
                                              _cutPlantBloc
                  ...
                  } else if (state is CutPlantNoData) {
                    widgetBox = Column(
                      children: [
                        Text("ไม่มีข้อมูล"),
                      ],
                    );
                  } else if (state is CutPlantError) {
                    widgetBox = Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                  ...
                  } else if (state is CutPlantLoading) {
                    widgetBox = Column(
                      children: [
                        Center(
                          child: new Container(
                  ...
                  return Column(
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          _cutPlantBloc.startDate == null
                              ? SizedBox()
                              : PickTime(
                                  startDate: _cutPlantBloc.startDate,
                                  endDate: _cutPlantBloc.endDate,
                                  focusedStartDay: (value) {
                                    _cutPlantBloc
                                        .add(UpdateCutPlantStartDate(value));
                                  },
                                  focusedLastDay: (value) {
                                    print(value);
                                    _cutPlantBloc
                                        .add(UpdateCutPlantEndDate(value));
                                  },
                                ),
                          _cutPlantBloc.stationSelect == null
                              ? SizedBox()
                              : Container(
                                  height: 40,
                                  width: 100,
                                  ...
                                  child: DropdownButton<PlotDetail>(
                                    value: _cutPlantBloc.stationSelect,
                                    ...
                                    onChanged: (PlotDetail newValue) {
                                      _cutPlantBloc
                                          .add(UpdateCutPlantStation(newValue));
                                    },
                                    items: _cutPlantBloc.stationList
