...
part 'seed_plant_event.dart';
part 'seed_plant_state.dart';

class SeedPlantBloc extends Bloc<SeedPlantEvent, SeedPlantState> {
  SeedPlantBloc() : super(SeedPlantInitial());
  ...
  @override
  Stream<SeedPlantState> mapEventToState(
    SeedPlantEvent event,
  ) async* {
    if (event is LoadSeedPlantInitial) {
      yield SeedPlantInitial();
    } else if (event is LoadSeedPlantData) {
      _mapLoadSeedPlantState(event);
      yield* _connectAPI();
    } else if (event is UpdateSeedPlantStation) {
      stationSelect = event.stationSelected;
      yield* _connectAPI();
    } else if (event is UpdateSeedPlantStartDate) {
      startDate = event.startDate;
      yield* _connectAPI();
    } else if (event is UpdateSeedPlantEndDate) {
      endDate = event.endDate;
      yield* _connectAPI();
    } else if (event is DeleteSeedPlant) {
  ...
  _mapLoadSeedPlantState(LoadSeedPlantData event) {
  ...
  Stream<SeedPlantState> _connectAPI() async* {
    yield SeedPlantLoading();
    try {
      //cut
      ...
      var rs = await ActivityAPI().getGrowView(
        "63_64",
      ...
      if (jsonRes.isNotEmpty) {
        ...
        print("success");
        yield SeedPlantLoaded();
      } else {
        yield SeedPlantNoData();
      }
    } catch (e) {
      yield SeedPlantError(e.toString());
    }
  }
}
