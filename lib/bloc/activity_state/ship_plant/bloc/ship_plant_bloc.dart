...
part 'ship_plant_event.dart';
part 'ship_plant_state.dart';

class ShipPlantBloc extends Bloc<ShipPlantEvent, ShipPlantState> {
  ShipPlantBloc() : super(ShipPlantInitial());
  ...
  @override
  Stream<ShipPlantState> mapEventToState(
    ShipPlantEvent event,
  ) async* {
    if (event is LoadShipPlantInitial) {
      yield ShipPlantInitial();
    } else if (event is LoadShipPlantData) {
      _mapLoadShipPlantState(event);
      yield* _connectAPI();
    } else if (event is UpdateShipPlantStation) {
      stationSelect = event.stationSelected;
      yield* _connectAPI();
    } else if (event is UpdateShipPlantStartDate) {
      startDate = event.startDate;
      yield* _connectAPI();
    } else if (event is UpdateShipPlantEndDate) {
      endDate = event.endDate;
      yield* _connectAPI();
    } else if (event is DeleteShipPlant) {
  ...
  _mapLoadShipPlantState(LoadShipPlantData event) {
  ...
  Stream<ShipPlantState> _connectAPI() async* {
    yield ShipPlantLoading();
    try {
      //cut
      ...
      var rs = await ActivityAPI().getTransportView(
        "63_64",
      ...
      if (jsonRes.isNotEmpty) {
        ...
        print("success");
        yield ShipPlantLoaded();
      } else {
        yield ShipPlantNoData();
      }
    } catch (e) {
      yield ShipPlantError(e.toString());
    }
  }
}
