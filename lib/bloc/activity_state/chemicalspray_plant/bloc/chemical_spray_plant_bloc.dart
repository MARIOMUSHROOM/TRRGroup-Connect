...
part 'chemical_spray_plant_event.dart';
part 'chemical_spray_plant_state.dart';

class ChemicalSprayPlantBloc
    extends Bloc<ChemicalSprayPlantEvent, ChemicalSprayPlantState> {
  ChemicalSprayPlantBloc() : super(ChemicalSprayPlantInitial());
  ...
  @override
  Stream<ChemicalSprayPlantState> mapEventToState(
    ChemicalSprayPlantEvent event,
  ) async* {
    if (event is LoadChemicalSprayPlantInitial) {
      yield ChemicalSprayPlantInitial();
    } else if (event is LoadChemicalSprayPlantData) {
      _mapLoadChemicalSprayPlantState(event);
      yield* _connectAPI();
    } else if (event is UpdateChemicalSprayPlantStation) {
      stationSelect = event.stationSelected;
      yield* _connectAPI();
    } else if (event is UpdateChemicalSprayPlantStartDate) {
      startDate = event.startDate;
      yield* _connectAPI();
    } else if (event is UpdateChemicalSprayPlantEndDate) {
      endDate = event.endDate;
      yield* _connectAPI();
    } else if (event is DeleteChemicalSprayPlant) {
  ...
  _mapLoadChemicalSprayPlantState(LoadChemicalSprayPlantData event) {
  ...
  Stream<ChemicalSprayPlantState> _connectAPI() async* {
    yield ChemicalSprayPlantLoading();
    try {
      //cut
      ...
      var rs = await ActivityAPI().getSprayView(
        "63_64",
      ...
      if (jsonRes.isNotEmpty) {
        ...
        print("success");
        yield ChemicalSprayPlantLoaded();
      } else {
        yield ChemicalSprayPlantNoData();
      }
    } catch (e) {
      yield ChemicalSprayPlantError(e.toString());
    }
  }
}
